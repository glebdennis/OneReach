...
- The data set in *input value* and *condition value* fields  is the data that is being compared. They are represented by white rectangles inside of the ***condition rule*** rectangle, *input value* on the left, *condition value* on the right. Depending on the chosen *data type*, this data can be filled in plain text mode either manually (for *"string"* and *"number"*, ) or with a *merge field*, or in *code mode* (for *"object or array"* and *"advanced"*):. Click inside the rectangle for the former or click on **{x}** for the latter.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTIxNzU4MTQ1MSwxMDIwNTU4ODIyXX0=
-->